---
description: Evaluate implementation quality against Go standards
allowed-tools:
  - Read
  - Grep
  - Glob
...
# Evaluate PRP Implementation Quality
...
Evaluate the code and documentation produced by executing a PRP against quanta's Go standards and idiomatic patterns. This command should be run AFTER execute-prp.md has completed implementation.

**Output Files**:

- Evaluation Report: `docs/PRPs/{input_name_without_extension}-evaluation.md`
- Fix PRP (if score < 8): `docs/PRPs/{input_name_without_extension}-fixes.md`

Example: If evaluating `status-command-feature.md`:

- Creates: `docs/PRPs/status-command-feature-evaluation.md`
- May create: `docs/PRPs/status-command-feature-fixes.md`
...
```bash
# After running execute-prp
/execute-prp status-command

# Evaluate the implementation (saves to docs/PRPs/status-command-evaluation.md)
/evaluate-prp status-command

# If issues found, execute fixes
/execute-prp status-command-fixes
```

Remember: This evaluation focuses on the ACTUAL CODE produced, not the PRP document. The goal is to ensure implementation meets quanta's Go standards before merging.
