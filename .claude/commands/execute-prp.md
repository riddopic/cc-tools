---
description: Execute PRP using sub-agent orchestration for Go implementation
allowed-tools:
  - Read
  - Grep
  - Glob
...
|-----------|-------------|---------|
| **PRP Name** | The PRP filename without extension | `feedback-driven-exploitation` |
...
| `multi-model-ensemble` | `docs/PRPs/multi-model-ensemble.md` |
| `unified-tui-interface` | `docs/PRPs/unified-tui-interface.md` |
...
**Objective**: Implement a Go feature following the PRP file using intelligent sub-agent orchestration with strict adherence to quanta's Go coding standards and idiomatic patterns.
...
### Phase 0: Task Subsystem Initialization

Before any implementation, create all PRP tasks using TaskCreate with dependency mapping:

1. Parse the PRP's task list (the `### List of tasks` section)
2. For each task, call TaskCreate with:
   - **subject**: Task name from PRP (e.g., "Task 1: Define Interfaces")
   - **description**: The task's full description, files, acceptance criteria, and commit message
3. After all tasks are created, set up dependencies using TaskUpdate:
   - Map each task's `depends_on` field to `addBlockedBy` relationships
   - Tasks with no dependencies can potentially run in parallel
4. Run TaskList to display the full task graph before starting work

This task graph serves as the progress tracker and recovery point for the entire execution.

### Phase 1: Requirements Analysis & Planning

First use the **deep-research-specialist** sub-agent to:

- Load and analyze the PRP file at `docs/PRPs/$ARGUMENTS.md`
- Review all Go context in `docs/CODING_GUIDELINES.md` and `docs/examples/` directory
- Research any unfamiliar Go packages or libraries mentioned
- Document all requirements and technical considerations

Then use the **product-manager-orchestrator** sub-agent to:

- Break down the PRP into specific implementation tasks
- Categorize requirements by package (cmd/, internal/, pkg/)
- Create an interface-first implementation strategy
- Define success criteria based on PRP objectives
...
### Phase 4.5: Atomic Commit & Progress Checkpoint

After each task's sub-agent completes and quality gates pass:

1. **Commit atomically**: Stage only the files for this task and commit using the task's suggested commit message
   ```bash
   git add <task-specific-files>
   git commit -m "<commit message from PRP task>"
   ```
2. **Mark task completed**: `TaskUpdate(taskId, status: "completed")`
3. **Show progress**: Run `TaskList` to display remaining work and unblocked tasks
4. **Pick next task**: Select the next task whose `blockedBy` dependencies are all completed

Repeat Phase 4 → Phase 4.5 for each task until all tasks are implemented.
...
The PRP is successfully executed when:

1. **All PRP requirements implemented** - Every feature/fix specified is complete
2. **Interface-first design** - Interfaces defined before implementations
3. **Table-driven tests** - Comprehensive test coverage with subtests
...
Throughout execution, use the task subsystem for progress tracking:
...
The workflow continues until all tasks show `completed` in TaskList or maximum iterations are reached.

## Recovery Protocol

If the session fails mid-execution (context overflow, crash, timeout):

1. **Start a new session** with the PRP file pinned: `/execute-prp <prp-name>`
2. **Check git log** to see which tasks were already committed atomically
3. **Run TaskList** — if tasks were created in the failed session, they show remaining work
4. **Skip completed tasks** — only execute tasks not yet committed
5. The PRP serves as the stable recovery point since it contains the full task graph with dependencies
...
**Final Step**: Read and execute `.claude/commands/evaluate-prp.md`
