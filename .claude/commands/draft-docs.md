...
```
/draft-docs "Document the authentication middleware"
```
...
1. `docs-style` - Core writing principles
2. Detected type skill:
   - `reference-docs` for Reference
   - `howto-docs` for How-To
...
4. **Report to user:**
   ...
   **Based on:** [list of analyzed symbols/files]
   ...
   3. When ready, publish with:
      ```
      /draft-docs --publish docs/drafts/{slug}.md
      ```
   ```
...
```
/draft-docs --publish docs/drafts/websocket-api.md
```
