...
### Go Architecture Explanation Pattern

For explaining Go system architecture:
...
We chose a pipeline architecture because vulnerability detection naturally
flows through discrete stages, each building on the previous:

```
Bytecode → Parser → CFG → Detectors → Exploits
```

Each stage is independent and testable, following Go's interface composition:
...
**Prioritized**: Testability, extensibility, clear data flow
**Sacrificed**: Some performance (data copying between stages)

This trade-off makes sense because:
- Security tools must be auditable and testable
- New detection strategies are added frequently
- Analysis correctness matters more than speed

## Design Decisions

### Interface-Based Composition

Each detector implements a common interface, allowing:
...
- [docs/A1-ASCEG-PRD.md](../../../docs/A1-ASCEG-PRD.md) - System requirements
- [coding-philosophy skill](../coding-philosophy/) - Architectural decision patterns (LEVER)
