...
4. **Gather Evidence in Multi-Component Systems**
   ...
   **Example (multi-layer system):**
   ...
   zap.L().Debug("config loaded",
       ...
       zap.Bool("tor_enabled", cfg.TorEnabled),
   )

   // Layer 2: Client creation
   client, err := cmd.CreateHTTPClient()
   zap.L().Debug("http client created",
       zap.Bool("tor_proxy", client.Transport != http.DefaultTransport),
       zap.Error(err),
   )

   // Layer 3: RPC connection
   zap.L().Debug("connecting to RPC",
       zap.String("url", rpcURL),
   )
   ethClient, err := ethclient.Dial(rpcURL)

   // Layer 4: Contract interaction
   zap.L().Debug("calling contract",
       zap.String("address", contractAddr.Hex()),
       zap.String("method", methodName),
   )
   ```

   **This reveals:** Which layer fails (config → client ✓, client → RPC ✗)
...
| **runtime/debug** | Stack traces | `debug.Stack()` for current goroutine |
| **GODEBUG** | Runtime diagnostics | `GODEBUG=gctrace=1 go test ./...` |
