...
#
# Options:
#   --project-dir <path>  Store session files under <path>/.cc-tools/brainstorm/
...
if [[ -n "$PROJECT_DIR" ]]; then
  SCREEN_DIR="${PROJECT_DIR}/.cc-tools/brainstorm/${SESSION_ID}"
else
...
if [[ ! -d "$SCRIPT_DIR/node_modules" ]]; then
  (cd "$SCRIPT_DIR" && npm install --silent 2>/dev/null) || {
    echo '{"error": "Failed to install dependencies. Run: cd '"$SCRIPT_DIR"' && npm install"}'
    exit 1
  }
fi
